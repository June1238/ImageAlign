...
if __name__ == '__main__':
    ...
    img1 = cv.imread(imgname1, cv.IMREAD_GRAYSCALE)  # 查询图片
    img2 = cv.imread(imgname2, cv.IMREAD_GRAYSCALE)  # 训练(目标)图片
    warped_image,_,_ = function_Alignment(img1,img2)
    dim = (800, 600)
    resized_image = cv.resize(warped_image, dim, interpolation=cv.INTER_AREA)
    # 显示图片
